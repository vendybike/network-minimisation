...
import org.sedaq.scp.service.exceptions.TabuSearchAlgorithmException;
...
public class TabuSearch implements MetaHeuristicAlgorithm {
    ...
    @Autowired
    public TabuSearch(WriteToCSVFile writeToCSVFile, RepairOperator repairOperator) {
    ...
    public List<Integer> startAlgorithm(InputMatrix inputMatrix, List<Integer> necessaryServiceCentres) {
        return startAlgorithm(inputMatrix, necessaryServiceCentres, 10000L, 1.0, Paths.get("./network-minimisation-service/src/main/resources/results/ts/"));
    }
    ...
    public List<Integer> startAlgorithm(InputMatrix inputMatrix, List<Integer> necessaryServiceCentres, long numberOfIterations, Double coveragePercentage, Path resultsBasePath) {
        ...
        Neighbour theBestCandidate = theBest;

        Deque<Neighbour> tabuList = new ArrayDeque<>();
        tabuList.add(theBestCandidate);

        long maxTabuSize = neighbours.getNeighbours().size();
        for (int i = 0; i < numberOfIterations; i++) {
            NeighbourHood sNeighbourHood = new NeighbourHood(generateNeighbourHood(inputMatrix, theBestCandidate, necessaryServiceCentres, coveragePercentage));
            for (Neighbour sCandidate : sNeighbourHood.getNeighbours()) {
                if ((sCandidate.getFitnessValue() < theBestCandidate.getFitnessValue()) && !tabuList.contains(sCandidate)) {
                    theBestCandidate = sCandidate;
                }
            }
            if (theBestCandidate.getFitnessValue() < theBest.getFitnessValue()) {
    ...
    private int computeFitnessValue(Neighbour neighbour, List<Integer> costs) {
        ...
        if (neighbourData == null) {
            throw new TabuSearchAlgorithmException("it is not possible to compute fitness because the neighbour was not generated..");
        }
        int sum = 0;
    ...
    public void writeResultsToTheFile(Path resultsBasePath, List<Integer> currentOccupantList, Neighbour currentOccupant) {
        ...
        Path results = Paths.get(resultsBasePath.toAbsolutePath().toString() + "/" + currentTime + "-ts-results-algorithm-iterations" + ".csv");
...
//        Path serviceCentreNamesFile = Paths.get("./network-minimisation-service/src/main/resources/results/services/service-centres-names.txt");
        Path resultList = Paths.get(resultsBasePath.toAbsolutePath().toString() + "/" + currentTime + "-ts-results-info" + ".txt");
